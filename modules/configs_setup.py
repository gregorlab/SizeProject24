...
def get_default_configs_path():
    ...
    This function computes the absolute path to the YAML configuration file named
    'default_configs.yml' stored within the 'configs' directory of the root
    'SizeIF' repository. The root directory is determined by navigating up the
    directory tree until reaching the prefix split before 'sizeIF'.

    :raises ValueError: If the directory structure does not contain 'sizeIF'.
    :raises FileNotFoundError: If the resulting configuration file path does not
                               exist.
    ...
    """
    cwd = os.getcwd()
    yaml_path = cwd.split('sizeIF')[0] + 'sizeIF/configs/default_configs.yml'
...
def get_default_plot_param_path():
    ...
    The function retrieves the current working directory of the program, splits its
    path string at the specified directory delimiter ('sizeIF'), and appends
    a pre-defined sub-path to assemble the full path to the YAML configuration file.

    :return: Full filesystem path to the default plotting parameter configuration file
    :rtype: str
    """
    cwd = os.getcwd()
    yaml_path = cwd.split('sizeIF')[0] + 'sizeIF/configs/default_plot_param.yml'
